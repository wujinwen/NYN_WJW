...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        [_sureBtn setTitleColor:[UIColor grayColor]
                       forState:UIControlStateNormal];
...
- (void)loadAddressData{
    ...
    for (NSDictionary *dic in _dataDict) {
        NYNProvinceModel *model = [NYNProvinceModel mj_objectWithKeyValues:dic];
...
numberOfRowsInComponent:(NSInteger)component{
    ...
    if (0 == component) {
        ...
        NYNProvinceModel  * p    = self.addressDataArr[selectProvince];
        ...
        NYNProvinceModel  * p    = self.addressDataArr[selectProvince];
        NYNCityModel *c = p.citys[selectCity];
//        NYNMergerModel *mModel = c.countys[];
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (0 == component) {
        NYNProvinceModel * p = self.addressDataArr[row];
        return p.name;
    }
    else if (1 == component) {
        NYNProvinceModel * p = self.addressDataArr[[pickerView selectedRowInComponent:0]];
        if (row > p.citys.count - 1) {
            return nil;
        }
        NYNCityModel * selectP = p.citys[row];

        return selectP.name;
    }
    else if (2 == component) {
        NYNProvinceModel * p = self.addressDataArr[[pickerView selectedRowInComponent:0]];
        NYNCityModel * selectP = p.citys[[pickerView selectedRowInComponent:1]];
        
        if ([pickerView selectedRowInComponent:0] > self.addressDataArr.count - 1) {
            return nil;
        }
        if ([pickerView selectedRowInComponent:1] > p.citys.count - 1) {
            return nil;
        }
        if ([pickerView selectedRowInComponent:2] > selectP.countys.count - 1) {
            return nil;
        }
        NYNMergerModel *megerModel = selectP.countys[row];

        return megerModel.name;
    }
    return nil;
}
...
- (void)sureBtnClicked{
    ...
    if ([_delegate respondsToSelector:@selector(sureBtnClickReturnProvince:City:Area:latModel:ProvinceID:CityID:)]) {
        ...
        NYNProvinceModel *pModel = self.addressDataArr[selectProvince];
        NYNCityModel *CModel = pModel.citys[selectCity];
        NYNMergerModel *MModel = CModel.countys[selectArea];

//        Province * p = _pArr[selectProvince];
        //解决省市同时滑动未结束时点击完成按钮的数组越界问题
        if (selectCity > pModel.citys.count - 1) {
            selectCity = pModel.citys.count - 1;
        }
        
        //解决省市区同时滑动未结束时点击完成按钮的数组越界问题
        if (selectArea > CModel.countys.count - 1) {
            selectArea = CModel.countys.count - 1;
        }
        [_delegate sureBtnClickReturnProvince:pModel.name
                                         City:CModel.name
                                         Area:MModel.name
                                         latModel:MModel
                                         ProvinceID:pModel.ID
                                         CityID:CModel.ID];
    }
}
