...
- (void)showPhotoBrowser
{
    ...
    //CHLog(@"===targetTemp:%@",NSStringFromCGRect(targetTemp));
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([parentView isKindOfClass:[UIView class]]) {
        if (self.indexNumber==2) {
            targetTemp.origin.y =  targetTemp.origin.y + 85*CHScaleX*0.5;
        }
        else
        {
          targetTemp.origin.y =  targetTemp.origin.y + 85*CHScaleX;  
        }
    ...
    JZLog(@"targetTemp.frame:%@",NSStringFromCGRect(targetTemp));
