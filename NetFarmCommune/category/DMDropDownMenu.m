...
- (void)tapAction
{
    ...
    if (_isOpen) {
        if ([_delegate respondsToSelector:@selector(menuWillChange:height:)]) {
            [_delegate menuWillChange:NO height:0];
        }
        _isOpen = NO;
    ...
    {
        if ([_delegate respondsToSelector:@selector(menuWillChange:height:)]) {
            [_delegate menuWillChange:YES height:tableH];
        }
        _isOpen = YES;
        [UIView animateWithDuration:0.3 animations:^{
