...
- (void)viewDidLoad {
    ...
    NSDictionary *dic = @{@"pageNo":@"1",@"pageSize":@"10",@"orderType":@"multiple",@"type":@"live"};
...
//    myMovieVC.farmString = [NSString stringWithFormat:@"热门直播>%@",model.farmTitle];
//
...
//    [self.navigationController pushViewController:myMovieVC animated:YES];
    ...
    personVC.LiveID= model.ID;
        if ([model.type isEqualToString:@"live"]) {
            personVC.islive =YES;
        personVC.chatId =[NSString stringWithFormat:@"%@",model.ID];
...
-(UICollectionView *)liveListCollectionView{
    if (!_liveListCollectionView) {
        ...
        _liveListCollectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT - 64)collectionViewLayout:flowLayout];
