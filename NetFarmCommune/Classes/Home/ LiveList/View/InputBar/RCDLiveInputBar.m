...
//弹幕按钮协议
-(void)catSwitchButton:(BOOL)state{
    if ([self.delegate respondsToSelector:@selector(catchSwitchDanm:)]) {
         [self.delegate catchSwitchDanm:state];
    }
   
    

//    self.allowDanMu = !self.allowDanMu;
}
/**
 *  设置输入框初始状态
 *
...
- (void)didTouchKeyboardReturnKey:(RCDLiveInputView *)inputControl
    ...
    if([self.delegate respondsToSelector:@selector(onTouchSendButton:)]){
          [_chatSessionInputBarControl.inputTextView resignFirstResponder];
...
-(void)setHideDanMu:(BOOL)hideDanMu {
    self.chatSessionInputBarControl.hideDanMu = hideDanMu;
}
