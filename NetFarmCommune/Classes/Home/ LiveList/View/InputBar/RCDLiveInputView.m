...
@interface RCDLiveInputView () <UITextViewDelegate,catSwitchDelegate>
...
- (void)resetInputBar {
    ...
    if (self.catswitch) {
        [self.catswitch removeFromSuperview];
        self.catswitch = nil;
    }
...
- (void)configInputContainerView {
    ...
    if (!self.hideDanMu) {
        self.catswitch = [[catSwitch alloc]initWithFrame:CGRectMake(3, 8, 50, 40)];
           self.catswitch.delegate=self;
        [self.inputContainerView addSubview:_catswitch];
    }
    ...
    [_emojiButton setTitle:@"发送" forState:UIControlStateNormal];
    [_emojiButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    _emojiButton.titleLabel.font = [UIFont systemFontOfSize:14];
...
- (void)setLayoutForInputContainerView {
    self.catswitch.translatesAutoresizingMaskIntoConstraints = NO;
    ...
    NSString *format =  @"H:|-0-[_switchButton(0)]-60-[_inputTextView]-10-[_emojiButton(30)" @"]-10-|";
...
//    [self.delegate didTouchEmojiButton:sender];
    [self.delegate didTouchKeyboardReturnKey:self text:_inputTextView.text];
...
-(void)switchState:(BOOL)state{
    [self.delegate catSwitchButton:state];
...
-(void)setHideDanMu:(BOOL)hideDanMu {
    self.catswitch.hidden = hideDanMu;
}
