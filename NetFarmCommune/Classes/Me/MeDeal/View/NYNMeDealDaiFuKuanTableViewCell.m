//
//  NYNMeDealDaiFuKuanTableViewCell.m
//  NetFarmCommune
//
...
@interface NYNMeDealDaiFuKuanTableViewCell()
...
@implementation NYNMeDealDaiFuKuanTableViewCell
...
- (void)awakeFromNib {
    ...
    shouhuozhuangtaiLabel.text = @"待付款";
...
//    chakanwuliubt.tag = 1;
    ...
    [pingjiabt setTitle:@"付款" forState:0];
...
- (void)click:(UIButton *)sender{
    ...
    if (self.ccblock) {
        self.ccblock(i,_model);
    }
}
-(void)setModel:(GoodsDealModel *)model{
    _model = model;
    _nongchangLabel.text = model.farm[@"name"];
    _titleLabel.text = model.orderItems[0][@"name"];
    
    
    _createTimeLabel.text =[NSString stringWithFormat:@"创建时间：%@",[MyControl timeWithTimeIntervalString:model.createDate]] ;
    
    [_viewOne sd_setImageWithURL:[NSURL URLWithString:model.farm[@"img"]] placeholderImage:[UIImage imageNamed:@"占位图"]];

    _contentLabel.text =model.orderItems[0][@"cname"];
    
    _titleLabel.text=model.orderItems[0][@"name"];
    _contentLabel.text =model.orderItems[0][@"cname"];
    _numLable.text =[NSString stringWithFormat:@"数量：%@", model.orderItems[0][@"quantity"]];
   [ _huoImageView sd_setImageWithURL:[NSURL URLWithString:model.orderItems[0][@"thumbnail"]] placeholderImage:[UIImage imageNamed:@"占位图"]];
    _yunfeiLabel.text = [NSString stringWithFormat:@"合计：¥%@（含运费¥%@）",model.amount,model.freight];

     _singlePrice.text = [NSString stringWithFormat:@"%@",model.orderItems[0][@"price"]];
}
@end
