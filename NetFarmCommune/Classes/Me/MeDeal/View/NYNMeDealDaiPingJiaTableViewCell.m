//
//  NYNMeDealDaiPingJiaTableViewCell.m
//  NetFarmCommune
//
...
#import "NYNMeDealDaiPingJiaTableViewCell.h"

@implementation NYNMeDealDaiPingJiaTableViewCell
...
-(void)setMoedel:(GoodsDealModel *)moedel{
    _moedel = moedel;
    _nongName.text = moedel.farm[@"name"];
    [_viewOne sd_setImageWithURL:[NSURL URLWithString:moedel.farm[@"img"]] placeholderImage:[UIImage imageNamed:@"占位图"]];
    
    if ([moedel.status isEqualToString:@"canceled"]) {
        _shouhuozhuangtaiLabel.text =@"交易取消";
        
    }else if ([moedel.status isEqualToString:@"pendingPayment"]){
        _shouhuozhuangtaiLabel.text =@"等待付款";
    }else if ([moedel.status isEqualToString:@"pendingReview"]){
        _shouhuozhuangtaiLabel.text =@"等待审核";
    }
    else if ([moedel.status isEqualToString:@"pendingShipment"]){
        _shouhuozhuangtaiLabel.text =@"等待发货";
    } else if ([moedel.status isEqualToString:@"shipped"]){
        _shouhuozhuangtaiLabel.text =@"已发货";
    }
    ...
    [_jiantouV sd_setImageWithURL:[NSURL URLWithString:moedel.orderItems[0][@"thumbnail"]] placeholderImage:[UIImage imageNamed:@"占位图"]];
    _titleLabel.text=moedel.orderItems[0][@"name"];
    _contentLabel.text =moedel.orderItems[0][@"cname"];
    _numLable.text =[NSString stringWithFormat:@"x%@", moedel.orderItems[0][@"quantity"]];
}
