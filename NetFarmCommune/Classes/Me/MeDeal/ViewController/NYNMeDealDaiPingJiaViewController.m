//
//  NYNMeDealDaiPingJiaViewController.m
//  NetFarmCommune
//
...
@interface NYNMeDealDaiPingJiaViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation NYNMeDealDaiPingJiaViewController
...
-(void)getData{
    ...
    NSDictionary * ns =  @{@"pageNo":@"1",@"pageSize":@"10",@"type":@"general",@"status":@"received"};;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NYNMeDealDaiPingJiaTableViewCell *farmLiveTableViewCell = [tableView dequeueReusableCellWithIdentifier:@"activityCell"];
    if (farmLiveTableViewCell == nil) {
        farmLiveTableViewCell = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([NYNMeDealDaiPingJiaTableViewCell class]) owner:self options:nil].firstObject;
    }
    farmLiveTableViewCell.ccblock = ^(NSInteger i) {
        if (i == 0) {
            ...
            //评价点击
            PinJiaViewController * pinjiaVC = [[PinJiaViewController alloc]init];
            [self.navigationController pushViewController:pinjiaVC animated:YES];
        }
        
    };
    farmLiveTableViewCell.moedel=_dataListArr[indexPath.row];
