...
- (void)configTable{
    ...
    [self.mTable registerNib:[UINib nibWithNibName:@"PullListTableViewCell" bundle:nil] forCellReuseIdentifier:@"PullListTableViewCell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cellModel.isCell ==YES) {
     
        PullListTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"PullListTableViewCell" forIndexPath:indexPath];
         cell.menuModel = cellModel;
        cell.zwPullMenuStyle = self.zwPullMenuStyle;

        cell.isFinalCell = (indexPath.row == self.menuArray.count-1);
        cell.listImageView.layer.cornerRadius = 25;
...
-(void)setMenuArray:(NSArray<ZWPullMenuModel *> *)menuArray{
    ...
    if (menuArray.count >0) {
        if ([menuArray[0] isKindOfClass:[ZWPullMenuModel class]]) {
            if ([(ZWPullMenuModel*)menuArray[0] isCell]) {
                self.menuCellHeight = 80;
